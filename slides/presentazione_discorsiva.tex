...
\begin{frame}{Problema Inverso}
  \begin{block}{Il problema che affrontiamo}
    Data un'immagine di citologia cervicale degradata da \textbf{blur gaussiano} e \textbf{rumore AWGN}, vogliamo stimare l'immagine originale.
  \end{block}
  \begin{equation*}
    y = \mathcal{H}(x) + n
  \end{equation*}
  \noindent dove $x$ \`e l'originale (sconosciuta), $\mathcal{H}$ \`e il blur ($\sigma=2$, kernel $9\times9$), $n$ \`e AWGN con $\sigma_n \in \{0.005, 0.01, 0.05, 0.1\}$.
  \vspace{0.15cm}
  \begin{alertblock}{Perch\'e \`e difficile}
    Il problema \`e \textbf{mal posto}: infinite immagini $x$ producono la stessa $y$. Serve un \textbf{prior}: TV lo codifica come regolarizzazione matematica, UNet lo apprende dai dati, DiffPIR lo genera con un modello di diffusione.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Metodi Scelti}
  ...
  \vspace{0.2cm}
  \noindent Abbiamo selezionato tre approcci fondamentalmente diversi, uno per ciascuna famiglia metodologica del corso. Ci\`o ci permette un confronto che non \`e solo numerico, ma anche qualitativo sulle diverse filosofie di risoluzione.
  \vspace{0.2cm}
  \begin{columns}[t]
...
\begin{frame}{Total Variation -- Teoria}
  ...
  \end{block}
  \vspace{0.15cm}
  \noindent L'idea \`e semplice: vogliamo un'immagine che, dopo blur, assomigli a $y$, ma che sia anche \emph{regolare}. La \textbf{Total Variation} misura proprio la regolarit\`a: somma le differenze assolute tra pixel adiacenti.
  \begin{equation*}
    TV(x) = \sum_{i,j} |x_{i+1,j} - x_{i,j}| + |x_{i,j+1} - x_{i,j}|
  \end{equation*}
  \noindent A differenza di Tikhonov, la TV \textbf{preserva i bordi}: penalizza le piccole oscillazioni (rumore) ma permette salti netti. Il costo \`e l'effetto \textit{staircasing} se $\lambda$ \`e troppo alto.
  \vspace{0.15cm}
  \begin{block}{Parametri scelti}
    Dopo validazione: $\lambda_{reg}=0.005$, ottimizzatore Adam ($lr=0.001$), 150 iterazioni, clamping $[-1,1]$ dopo ogni passo.
  \end{block}
\end{frame}
...
\begin{frame}{Confronto Visivo -- $\sigma_n = 0.1$}
  \begin{center}
  ...
  \vspace{0.1cm}
  \begin{center}
    \textbf{Mappe di differenza} ($|\hat{x} - x_{\text{GT}}|$, normalizzate)
  \end{center}
  \begin{center}
