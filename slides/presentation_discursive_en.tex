...
\date{A.Y. 2025/2026}
\titlegraphic{\includegraphics[width=0.22\textwidth]{figures/unibo_logo.png}}
\logo{\includegraphics[width=0.05\textwidth]{figures/unibo_logo.png}}
  ...
  \begin{columns}[t]
    \column{0.33\textwidth}
      \begin{block}{Variational}
        \centering\includegraphics[height=1.1cm]{../results/tv/qualitative/noise_0.005_sample0.png}\\[0.05cm]
    ...
    \column{0.33\textwidth}
      \begin{block}{Deep Learning}
        \centering\includegraphics[height=1.1cm]{../results/unet/qualitative/noise_0.005_sample0.png}\\[0.05cm]
        \textbf{UNet}
        \begin{itemize}\small
    ...
    \column{0.33\textwidth}
      \begin{block}{Generative}
        \centering\includegraphics[height=1.1cm]{../results/diffpir/qualitative/noise_0.1_sample0.png}\\[0.05cm]
        \textbf{DiffPIR}
        \begin{itemize}\small
    ...
    \column{0.5\textwidth}
      ...
      \begin{tabular}{l|ccc}\toprule
        $\sigma_n$ & \textbf{PSNR} & \textbf{SSIM} & \textbf{Time}\\\midrule
         0.005 & 29.58 & 0.896 & 0.013 s\\
         0.01  & 29.57 & 0.895 & 0.013 s\\
         0.05  & 29.37 & 0.874 & 0.012 s\\
         0.1   & 28.72 & 0.821 & 0.012 s\\\bottomrule
      \end{tabular}
      \vspace{0.3cm}
      \begin{itemize}
        \item Minimal degradation ($\sim$0.9 dB between extremes)
        \item \textbf{Surpasses TV} at $\sigma_n=0.1$ (28.72 vs 26.54 dB)
        \item Fastest inference: 0.013 s/img
      \end{itemize}
    \column{0.5\textwidth}
...
\begin{frame}{DiffPIR -- Overview}
  \begin{block}{Plug-and-Play: denoising prior + data fidelity}
    \begin{itemize}
      \item Integrates a \textbf{diffusion model} (DDPM) as the denoising prior
      \item Alternates \textbf{denoising} (generative prior) and \textbf{data-fidelity} (FFT closed-form)
      \item The prior is \textbf{independent} of the degradation model --- flexible framework
    \end{itemize}
  \end{block}
  \vspace{0.15cm}
  \begin{columns}[t]
    \column{0.5\textwidth}
      \textbf{Why diffusion?}\\[0.05cm]
      \small
      $\triangleright$ State-of-the-art generative prior\\[0.05cm]
      $\triangleright$ Handles complex image statistics better than TV
    \column{0.5\textwidth}
      \textbf{Key idea}\\[0.05cm]
      \small
      $\triangleright$ $x_t \xrightarrow{\text{DDPM}} \hat{x}_0$ (denoise)\\[0.05cm]
      $\triangleright$ $\hat{x}_0 \xrightarrow{\text{FFT}} \tilde{x}_0$ (fidelity)
  \end{columns}

  \vspace{0.12cm}
  \begin{exampleblock}{\centering\small\textbf{``Separate the prior from the physics''}}
    \centering\scriptsize
    DiffPIR = denoising (DDPM) $\leftrightarrow$ deblur (FFT) \\
    ``Plug-and-Play'': we can swap the denoiser without changing the fidelity term
  \end{exampleblock}
\end{frame}

\begin{frame}{DDPM + LightUNet: Training}
  \begin{columns}[t]
    \column{0.5\textwidth}
      \textbf{DDPM in a nutshell}\\[0.08cm]
      \small
      \begin{itemize}
        \item Forward: add Gaussian noise $x_0 \to x_T$ \\ (T=1000, $\beta_1=10^{-4}$, $\beta_T=0.02$)
        \item Reverse: neural network learns to \textbf{denoise} step by step \\ (predicts $\varepsilon_\theta(x_t,t)$)
        \item Loss: MSE on predicted noise
      \end{itemize}

      \vspace{0.25cm}
      \textbf{Our training setup}\\[0.08cm]
      \small
      \begin{itemize}
        \item 673 images, 50 epochs, batch=4
        \item MPS (Apple GPU), $\sim$10 min total
        \item \textbf{No pre-training} --- trained from scratch on LBC
      \end{itemize}

    \column{0.5\textwidth}
      \textbf{LightUNet architecture}\\[0.08cm]
      \small
      \begin{itemize}
        \item Base channels: 32
        \item Encoder: 32$\to$64$\to$128
        \item Decoder: 128$\to$64$\to$32
        \item Sinusoidal time embedding + MLP
        \item ResBlocks with GroupNorm + SiLU
      \end{itemize}

      \vspace{0.15cm}
      \begin{block}{\centering\small Total: \textbf{1.26M params} ($\sim$5 MB)}
        \centering\scriptsize Lightest model in the project (vs 1.9M UNet)
      \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}{DiffPIR Algorithm}
  \begin{columns}[t]
    \column{0.48\textwidth}
      \textbf{Pseudocode}\\[0.08cm]
      \small
      \begin{enumerate}
        \item $y \gets$ degraded image
        \item \textbf{for} $t = t_{\text{start}}$ \textbf{to} $1$ \textbf{do}
        \item \hspace{0.3cm}$\varepsilon \gets \underbrace{\varepsilon_\theta(x_t, t)}_{\text{DDPM predict noise}}$
        \item \hspace{0.3cm}$\hat{x}_0 \gets \frac{x_t - \sqrt{1-\bar{\alpha}_t}\,\varepsilon}{\sqrt{\bar{\alpha}_t}}$ \hfill \tiny denoise
        \item \hspace{0.3cm}$\tilde{x}_0 \gets \text{FFT-fidelity}(y, \hat{x}_0, \rho_t)$ \hfill \tiny deblur
        \item \hspace{0.3cm}$x_{t-1} \gets$ DDIM step$(x_t, \tilde{x}_0)$
        \item \textbf{end for}
        \item \textbf{return} $x_0$
      \end{enumerate}

      \vspace{0.2cm}
      \textbf{DDIM sampling:} $\zeta=0$ (deterministic)\\[0.05cm]
      15 steps subsampled from $t_{\text{start}}=50$

    \column{0.52\textwidth}
      \textbf{Step 1 --- Denoising (DDPM)}\\[0.08cm]
      \small
      Remove noise via the trained prior:
      \vspace{-0.05cm}
      \begin{equation*}
      ...
      \vspace{0.15cm}
      \textbf{Step 2 --- Data Fidelity (FFT)}\\[0.08cm]
      \small
      Enforce consistency with $y$ in frequency domain:
      \vspace{-0.05cm}
      \begin{equation*}
        \tilde{x}_0 = \mathcal{F}^{-1}\!\left(
        \frac{\mathcal{F}(y)\overline{\mathcal{F}(k)} + \rho_t\,\mathcal{F}(\hat{x}_0)}
             {|\mathcal{F}(k)|^2 + \rho_t}\right)
      \end{equation*}

      \vspace{0.15cm}
      \textbf{Dynamic weight} $\rho_t = \lambda \cdot \sigma^2 \cdot \alpha_t\,/\,(1 - \bar{\alpha}_t)$\\[0.05cm]
      \small
      \hspace{0.3cm}$\triangleright$ High $t$: prior dominates\\[0.02cm]
      \hspace{0.3cm}$\triangleright$ Low $t$: fidelity dominates
  \end{columns}
\end{frame}
...
\begin{frame}{DiffPIR -- Parameters and Results}
  \begin{columns}[T]
    \column{0.45\textwidth}
      ...
      \raggedright
      \begin{itemize}\small
        \item PSNR \textbf{increases} with noise (15.80 $\to$ 25.60 dB)
        \item Best at $\sigma_n=0.1$: surpasses TV (26.54 vs 25.60)\\[0.08cm]
        \textcolor{blue!60!black}{\small $\triangleright$ Low $\sigma_n$: model \textbf{hallucinates} details --- ``sees'' structures not in the signal}\\[0.02cm]
        \textcolor{blue!60!black}{\small $\triangleright$ High $\sigma_n$: generative prior is beneficial --- recovers what TV/UNet lose}
      \end{itemize}

    \column{0.55\textwidth}
      \centering
      \includegraphics[width=\textwidth, height=0.75\textheight, keepaspectratio]{../results/qualitative_slides/diffpir_comparison.png}
  \end{columns}
\end{frame}
...
\begin{frame}{PSNR / SSIM Comparison}
  ...
  \begin{tabular}{l|ccc|ccc}\toprule
    ...
    $\sigma_n$ & \textbf{TV} & \textbf{UNet} & \textbf{DiffPIR} & \textbf{TV} & \textbf{UNet} & \textbf{DiffPIR}\\\midrule
     0.005 & \textbf{32.09} & 29.58 & 15.80 & \textbf{0.911} & 0.896 & 0.356\\
     0.01  & \textbf{32.04} & 29.57 & 16.48 & \textbf{0.909} & 0.895 & 0.407\\
     0.05  & \textbf{30.42} & 29.37 & 22.77 & 0.837 & \textbf{0.874} & 0.722\\
     0.1   & 26.54 & \textbf{28.72} & 25.60 & 0.586 & \textbf{0.821} & 0.797\\\bottomrule
  \end{tabular}
  \vspace{0.3cm}
  \begin{block}{Observations}
    \begin{itemize}
      \item \textbf{TV}: dominates at $\sigma_n\leq0.01$ (PSNR $>$ 32 dB, SSIM $>$ 0.9)
      \item \textbf{UNet}: best trade-off, $\sim$0.9 dB degradation, surpasses TV at $\sigma_n=0.1$
...
\begin{frame}{Full Grid}
  \begin{center}
    \includegraphics[width=0.9\textwidth]{../results/qualitative_slides/all_methods_grid.png}
  \end{center}
\end{frame}
...
\begin{frame}{Visual Comparison -- $\sigma_n = 0.05$}
  \begin{center}
    \includegraphics[width=0.95\textwidth]{../results/qualitative_slides/noise_0.05_comparison.png}
  \end{center}
\end{frame}
  ...
  \begin{columns}[t]
    ...
    \column{0.48\textwidth}
      ...
      \begin{block}{\small\bfseries UNet  \hfill \scriptsize\itshape Deep Learning}
        \scriptsize
        \textcolor{green!50!black}{$\blacktriangleright$} \textbf{Pros}\\[0.05cm]
        \hspace{0.3cm}$\checkmark$ Very fast inference (0.013\,s/img)\\
...
\begin{frame}{Operating Regimes}
  \begin{center}
    \begin{tabular}{lccc}\toprule
      ...
      Inference speed           & $\sim$0.7 s & $\sim$\textbf{0.013 s} & $\sim$0.64 s\\\bottomrule
    \end{tabular}
  \end{center}
  \vspace{0.3cm}
