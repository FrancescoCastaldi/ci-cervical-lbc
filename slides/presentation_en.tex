...
\begin{frame}{Inverse Problem}
  \begin{block}{Task}
    Given a cervical cytology image degraded by \textbf{Gaussian blur} and \textbf{AWGN noise}, estimate the original image.
  \end{block}
  \begin{equation*}
    y = \mathcal{H}(x) + n
  \end{equation*}
  \begin{itemize}
    \item $x$: original image (ground truth, unknown)
    \item $\mathcal{H}$: Gaussian blur ($\sigma=2$, kernel $9\times9$)
    \item $n$: AWGN with $\sigma_n \in \{0.005, 0.01, 0.05, 0.1\}$
    \item $y$: degraded observation
  \end{itemize}
  \begin{alertblock}{Ill-Posed Problem}
    Infinite solutions $x$ compatible with $y$. Need \textbf{regularization} (TV) or \textbf{learned priors} (UNet, DiffPIR).
  \end{alertblock}
\end{frame}

\begin{frame}{Selected Methods}
  \begin{center}\small Group of 2 students $\to$ 3 out of 4 methods (excluded Weighted TV)\end{center}
  \begin{columns}[t]
...
\begin{frame}{Mendeley LBC Cervical Cancer}
  \begin{columns}
    \column{0.55\textwidth}
      ...
      \end{itemize}
      \begin{block}{Diagnostic Classes}
...
\begin{frame}{Degradation}
  ...
  \begin{columns}
    \column{0.45\textwidth}
      \textbf{Degradation pipeline:}
      \begin{enumerate}\small
        \item Apply Gaussian blur $H$ to GT image $x$
        \item Add AWGN noise $n$ with deviation $\sigma_n$
        \item Clamp to $[-1,1]$
      \end{enumerate}
      \vspace{0.1cm}
      \begin{equation*}
...
\begin{frame}{Total Variation -- Theory}
  ...
  \end{block}
  \vspace{0.2cm}
  \textbf{Total Variation}: sum of absolute differences between adjacent pixels
  \begin{equation*}
    TV(x) = \sum_{i,j} |x_{i+1,j} - x_{i,j}| + |x_{i,j+1} - x_{i,j}|
  \end{equation*}
  \vspace{0.2cm}
  \begin{itemize}
    \item Preserves edges, removes noise
    \item \textit{Staircasing} effect if $\lambda$ is too high
  \end{itemize}
  \vspace{0.2cm}
  \begin{block}{Parameters}
    \begin{tabular}{ll}
      $\lambda_{reg}=0.005$ & optimal balance \\
      Optimizer & Adam ($lr=0.001$) \\
      Iterations & 150 \\
      Clamping & $[-1,1]$ after each step \\
    \end{tabular}
  \end{block}
\end{frame}
  ...
  \begin{columns}
    \column{0.5\textwidth}
      ...
      \begin{itemize}
        ...
        \item Fastest inference: 0.035 s/img
      \end{itemize}
    \column{0.5\textwidth}
...
\begin{frame}{DiffPIR -- Overview}
  ...
  \begin{alertblock}{Custom LightUNet (1.26M params, $\sim$5 MB)}
    ...
    $\triangleright$ Training and inference on CPU
  \end{alertblock}
\end{frame}
  ...
  \begin{columns}
    \column{0.55\textwidth}
      ...
      \begin{itemize}\small
        ...
        \item Training: 673 images, 30 epochs
      \end{itemize}
      \vspace{0.2cm}
      \textbf{LightUNet}
      \begin{itemize}\small
...
\begin{frame}{DiffPIR -- Parameters and Results}
  \begin{columns}
    \column{0.45\textwidth}
      \textbf{Heuristic parameter selection}
      \begin{tabular}{ll}\toprule
        \textbf{Parameter} & \textbf{Value}\\\midrule
        $t_{\text{start}}$ & 50\\
        $\lambda$ & 10.0\\
      ...
      \begin{itemize}
        ...
        \item Best at $\sigma_n=0.1$ (25.46 dB)
      \end{itemize}
    \column{0.55\textwidth}
...
\begin{frame}{Implementation -- Repository Structure}
  \begin{columns}
    \column{0.55\textwidth}
      \textbf{Project tree}
      \vspace{0.1cm}
      ...
      \begin{tikzpicture}[scale=0.42, every node/.style={font=\scriptsize\ttfamily}]
        ...
        \node[anchor=west] at (0.6,-6.0) {tests/ (34 unit tests)};
    ...
    \column{0.45\textwidth}
      \textbf{Modularity}
      \begin{itemize}
        \item Each method is an independent module
        \item Shared pipeline: degradation, metrics
        \item Same test set for all
      \end{itemize}
      \vspace{0.3cm}
      \textbf{Execution:}
      ...
      \begin{block}{Reproducibility}
        Seed 42, shared pipeline, 34 unit tests
      \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}{PSNR / SSIM Comparison}
  ...
  \begin{block}{Observations}
    \begin{itemize}
      ...
      \item \textbf{DiffPIR}: generative, PSNR increases with noise (15.78$\to$25.46 dB)
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Visual Comparison -- $\sigma_n = 0.1$}
  \begin{center}
    \includegraphics[width=0.85\textwidth]{../results/qualitative_slides/methods_noise_0.1.png}
    \vspace{0.1cm}
    \includegraphics[width=0.65\textwidth]{../results/qualitative_slides/crop_diff_comparison.png}
  \end{center}
  \vspace{0.1cm}
  \begin{center}
    \textbf{Difference maps} ($|\hat{x} - x_{\text{GT}}|$, normalized)
  \end{center}
  \begin{center}
    \includegraphics[width=0.75\textwidth]{../results/diff/all_diffs_0.1.png}
  \end{center}
\end{frame}
  ...
  \begin{columns}[t]
    ...
    \column{0.48\textwidth}
      ...
      \begin{block}{\small\bfseries UNet  \hfill \scriptsize\itshape Deep Learning}
        \scriptsize
        \textcolor{green!50!black}{$\blacktriangleright$} \textbf{Pros}\\[0.05cm]
        \hspace{0.3cm}$\checkmark$ Very fast inference (0.035\,s/img)\\
  ...
  \begin{columns}[t]
    \column{0.70\textwidth}
      ...
      \begin{block}{\small\bfseries DiffPIR  \hfill \scriptsize\itshape Generative (Plug-and-Play)}
        ...
        \hspace{0.3cm}$\checkmark$ Improves with increasing noise (15.78\,$\to$\,25.46\,dB)\\[0.08cm]
        \textcolor{red}{$\blacktriangleright$} \textbf{Cons}\\[0.05cm]
        \hspace{0.3cm}$\times$  Slow ($\sim$3\,s/img) -- 15 DDIM steps\\
...
\begin{frame}{Operating Regimes}
  \begin{center}
    \begin{tabular}{lccc}\toprule
      ...
      Inference speed           & $\sim$7 s & $\sim$\textbf{0.035 s} & $\sim$3 s\\\bottomrule
    \end{tabular}
  \end{center}
  \vspace{0.3cm}
